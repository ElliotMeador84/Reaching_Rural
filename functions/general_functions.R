...
to_source_target <- function(i.graph, save_as_csv = F){
    ...
    if ( save_as_csv == T){
        pa <- paste0(getwd(),'/',deparse(substitute(i.graph)),'_edges.csv')
...
to_source_target_labels <- function(i.graph,save_as_csv = F){
   ...
   h <- bind_cols(e,f)
   if ( save_as_csv == T){
       pa <- paste0(getwd(),'/',deparse(substitute(i.graph)),'_nodes.csv')
       readr::write_csv(h,path = pa)
       print(message(paste('++++ file saved to',pa,'++++')))
   }
   h
